...
class forumsActions extends sfActions
{
  public function preExecute()
  {
    ...
    /**
     * Executes projectForumList action
     *
     */
    public function executeProjectForumList()
    {
      $this->tab = "forums";
      
      $categories = $this->project->getForumCategories();
      
      $nb_topics = 0;
      $nb_posts = 0;
      
      foreach($categories as $categories) {
        foreach ($categories->getFora() as $forums) {
          foreach($forums as $forum) {
            $nb_topics += $forum->getNbTopics();
            $nb_posts  += $forum->getNbPosts();
          }
    ...
    public function executePost()
    {
      ...
      $this->redirect($this->getModuleName().'/topic?id='.$topic->getId().'&stripped_title='.$topic->getStrippedTitle().'&project='.$this->project->getSlug().'&page='.$page.'#post'.$post->getId());
    }
    ...
    protected function setUserVars()
    {
      ...
      $this->logMessage('Checking for user by id: ['.$this->getRequestParameter('username').']');
      $this->user = sfSimpleForumTools::getUserByUuid($this->username);
      $this->logMessage('Finished checking for user...');
      $this->forward404Unless($this->user, 'User not found in forums, unable to set vars');
    }
    ...
    public function handleErrorAddTopic()
    {
      ...
      $this->forward('forums', 'createTopic');
    }
    ...
    {    
      ...
      if ($this->getUser()->isAuthenticated()) $this->getUser()->getProfile()->addKarma(sfConfig::get('app_karma_post_forum_points'));
    ...
    protected function redirectToPost($post)
    {
      $position = $post->getPositionInTopic();
      $page = ceil(($position + 1) / sfConfig::get('app_sfSimpleForumPlugin_max_per_page', 10));
      $this->redirect("@show_forum_topic?id=".$post->getTopic()->getId().'&project='.$this->getRequestParameter('project').'&forum_name='.$post->getsfSimpleForumForum()->getStrippedName().'&page='.$page.'#post'.$post->getId());
    }
