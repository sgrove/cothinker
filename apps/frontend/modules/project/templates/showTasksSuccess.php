...
<table id="task_table" border="0" class="grspd-wave" style="width:100%;">
  ...
  <tr style="background-color:#87B94D;">
    <th></th>
    <th><?php echo link_to('Task', '@list_project_tasks?project='.$project->getSlug().'&sort_by=name&direction='.$dir_opposite) ?></th>
    <th>Assigned to</th>
    <th><?php echo link_to('By', '@list_project_tasks?project='.$project->getSlug().'&sort_by=assigned_by&direction='.$dir_opposite) ?></th>
    <th><?php echo link_to('Due On', '@list_project_tasks?project='.$project->getSlug().'&sort_by=finish&direction='.$dir_opposite) ?></th>
    <th><?php echo link_to('Status', '@list_project_tasks?project='.$project->getSlug().'&sort_by=status&direction='.$dir_opposite) ?></th>
  </tr>
  </thead>
  <tbody>
...
<?php foreach ($tasks as $task): ?>
  ...
  <tr class="row-<?php echo $counter?> dynamic_status_<?php echo $task->getStatusInWords() ?>">
    ...
    <td><?php echo link_to_function($task->getName(), visual_effect('toggle_blind', 'task-details-'.$task->getUuid(), array('duration' => '0.5'))) ?> (<?php echo link_to('Details', '@show_project_task?project='.$project->getSlug().'&task='.$task->getUuid()) ?>)</td>
  ...
  <tr id="task-details-<?php echo $task->getUuid() ?>" style="display:none;" class="dynamic_status_<?php echo $task->getStatusInWords() ?>_details">
    ...
    <td colspan="5">
  		...
  		<p>
          <?php echo icon_tag('comment').' '.$task->getNbComments().' Comments ',
                      icon_tag('folder').' '.get_nb_files('Task', $task->getId()).' Files '; if ($sf_user->isAuthenticated() && $project->hasPermission('create-task', $sf_user->getId())) { echo icon_tag('note_edit').' '.link_to('Edit Task', '@edit_project_task?project='.$project->getSlug().'&task='.$task->getUuid()).' '.icon_tag('note_remove').' '.link_to('Delete Task', '@delete_project_task?project='.$project->getSlug().'&task='.$task->getUuid(), 'post=true&confirm=Are you sure you want to delete the task "'.$task->getName().'"?'); } ?>
  		</p>
  	</td>
  </tr>
<?php endforeach ?>
