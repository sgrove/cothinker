...
class nahoWikiActions extends sfActions
{
	public function preExecute()
  {
    ...
    $this->slug = $this->project->getSlug();
    
    $this->tab = sfConfig::get('app_tab_project_wiki');
  }
  ...
  protected function setPage($page_name = null)
  {
    ...
    if (substr($page_name, -1) == ':') {
		if ($this->getRequestParameter('wiki') == null) die('wiki not specified');
  ...
  protected function updateBreadcrumbs()
  {
    if (!$this->page->isNew()) {
      ...
      if (!in_array($this->page->getName(), $breadcrumbs)) {
        $breadcrumbs[] = array('wiki' => $this->wiki->getProject()->getSlug(), 'page' => $this->page->getName());
      }
  ...
  public function executeBrowse()
  {
    ...
    $this->tree = $tree;
    $this->uriParams = 'slug='.$this->slug.'&page=' . urlencode($this->getRequestParameter('page'));
