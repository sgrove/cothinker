...
<?php if (@$compare): ?>
  <?php echo form_tag('@wiki_diff?slug='.$page->getWiki()->getProject()->getSlug().'&page='.$page->getName(), 'name=diff_form id=diff_form class=wiki-form') ?>
  <?php echo input_hidden_tag('page', $page->getName()) ?>
  <?php echo input_hidden_tag('slug', $page->getWiki()->getProject()->getSlug()) ?>
<?php endif ?>
...
<table class="wiki-history">
  ...
  <?php foreach ($revisions as $revision): ?>
    <tr>
      ...
      <?php endif ?>
      <td><?php echo link_to_wiki($page->getWiki()->getProject()->getSlug(), $page->getWiki()->getProject()->getSlug(), $page->getName(), array('revision' => $revision->getRevision())) ?></td>
      <td><?php echo format_datetime($revision->getCreatedAt('U')) ?></td>
      <td><?php echo $revision->getUserName() //TODO: update revision model to link to sfGuardUser to allow for links to user pages: echo link_to_wiki_user($page->getWiki()->getProject()->getSlug(), $revision->getUserName(), $revision->getsfGuardUser()->getSlug()) ?></td>
