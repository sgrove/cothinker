...
abstract class BaseUserTask extends BaseObject  implements Persistent {
	...
	public function setUuid($v)
	{
		...
		if ($this->uuid !== $v) {
			$this->uuid = $v;
			$this->modifiedColumns[] = UserTaskPeer::UUID;
		}
	...
	public function setDeletedAt($v)
	{
		...
		if ($this->deleted_at !== $ts) {
			$this->deleted_at = $ts;
			$this->modifiedColumns[] = UserTaskPeer::DELETED_AT;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->task_id = $rs->getInt($startcol + 3);
    ...
    foreach (sfMixer::getCallables('BaseUserTask:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseUserTask:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseUserTask:save:pre') as $callable)
    {
    ...
    if ($this->isNew() && !$this->isColumnModified(UserTaskPeer::CREATED_AT))
    {
      $this->setCreatedAt(time());
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseUserTask:save:post') as $callable)
    {
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setTaskId($arr[$keys[3]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(UserTaskPeer::UUID)) $criteria->add(UserTaskPeer::UUID, $this->uuid);
		...
		if ($this->isColumnModified(UserTaskPeer::DELETED_AT)) $criteria->add(UserTaskPeer::DELETED_AT, $this->deleted_at);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(UserTaskPeer::USER_ID, $this->user_id);
		$criteria->add(UserTaskPeer::TASK_ID, $this->task_id);
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseUserTask:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseUserTask::%s', $method));
    }
