...
abstract class BasesfSimpleForumCategory extends BaseObject  implements Persistent {
	...
	public function setOwnerId($v)
	{
		...
		if ($this->owner_id !== $v) {
			$this->owner_id = $v;
			$this->modifiedColumns[] = sfSimpleForumCategoryPeer::OWNER_ID;
		}

		if ($this->asfSimpleForumOwner !== null && $this->asfSimpleForumOwner->getId() !== $v) {
			$this->asfSimpleForumOwner = null;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->owner_id = $rs->getInt($startcol + 1);

			$this->name = $rs->getString($startcol + 2);

			$this->stripped_name = $rs->getString($startcol + 3);
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			...
			if ($this->asfSimpleForumOwner !== null) {
				if ($this->asfSimpleForumOwner->isModified()) {
					$affectedRows += $this->asfSimpleForumOwner->save($con);
				}
				$this->setsfSimpleForumOwner($this->asfSimpleForumOwner);
			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if ($this->asfSimpleForumOwner !== null) {
				if (!$this->asfSimpleForumOwner->validate($columns)) {
					$failureMap = array_merge($failureMap, $this->asfSimpleForumOwner->getValidationFailures());
				}
			}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(sfSimpleForumCategoryPeer::OWNER_ID)) $criteria->add(sfSimpleForumCategoryPeer::OWNER_ID, $this->owner_id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(sfSimpleForumCategoryPeer::OWNER_ID, $this->owner_id);
	...
	public function setsfSimpleForumOwner($v)
	{
