...
abstract class BasenahoWikiPage extends BaseObject  implements Persistent {
	...
	public function setUuid($v)
	{
		...
		if ($this->uuid !== $v) {
			$this->uuid = $v;
			$this->modifiedColumns[] = nahoWikiPagePeer::UUID;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->wiki_id = $rs->getInt($startcol + 2);

			$this->name = $rs->getString($startcol + 3);

			$this->latest_revision = $rs->getInt($startcol + 4);
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getUuid(),
			$keys[2] => $this->getWikiId(),
			$keys[3] => $this->getName(),
			$keys[4] => $this->getLatestRevision(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[2], $arr)) $this->setWikiId($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setName($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setLatestRevision($arr[$keys[4]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(nahoWikiPagePeer::UUID)) $criteria->add(nahoWikiPagePeer::UUID, $this->uuid);
	...
	public function getnahoWikiWiki($con = null)
	{
		if ($this->anahoWikiWiki === null && ($this->wiki_id !== null)) {
						include_once 'lib/model/om/BasenahoWikiWikiPeer.php';
	...
	{
				include_once 'lib/model/om/BasenahoWikiRevisionPeer.php';
		if ($criteria === null) {
