...
class sfSimpleForumTools
{
  ...
  /**
   * Retrieves Propel user model instance from its uuid
   * 
   * @param string Uuid
   * @return mixed User model object (class sfGuard by default)
   */
  ...
  {
    sfContext::getInstance()->getLogger()->info('Checking in model layer for user with uuid: ['.$uuid.']');
    $user_class = sfConfig::get('app_sfSimpleForumPlugin_user_class', 'sfGuardUser');
    $user_peer = $user_class.'Peer';
    $method_name = sfConfig::get('app_sfSimpleForumPlugin_user_retrieve_by_name_method', 'retrieveByUuid');
    if (!is_callable($user_peer, $method_name))
    {
      throw new sfException(sprintf('Unexistant method %s::%s() for retrieving a %s from its username', $user_peer, $method_name, $user_class));
    }
    
    sfContext::getInstance()->getLogger()->info('Will call object::function(parameter): '.$user_peer.'::'.$method_name.'('.$uuid.')');
    
    return call_user_func(array($user_peer, $method_name), $uuid);
  }
